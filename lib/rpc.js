...
const RPC = ({ contact, handleCommand, logger = Logger() }) => {
	...
	const serverSocket = net.createServer(socket => {
		...
		socket.on('data', chunk => {
			...
			while (dIndex > -1) {
				...
				if (_.includes('MESSAGE_RESPONSE', command.strCommand)) {
	...
	const sendCommand = (command) => {
		return new Promise((resolve, reject) => {
			logger.debug(`${contact.username} attempting to connect to ${command.destination.username}`)
			...
			socket.on('connect', () => {
				...
				if (_.includes(['MESSAGE'], command.strCommand)) {
