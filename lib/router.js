  ...
  return new Promise((resolve, reject) => {
    ...
    const sendCommand = command => {
      const p = new Promise((resolve, reject) => {
        ...
        socket.on('error', err => {
          //console.log('port: ', command.destination.port);
          //console.log('ip: ', command.destination.ip);
          //console.log('command:\n', command);
    ...
    const handleCommand = (serializedData) => {
      ...
      switch(command.command)
      {
        ...
        case 'RETRIEVE_CONTACTS':
          ...
          }, wee1 => { console.log(wee1); throw wee1; }).then(() => {
          ...
          }, wee2 => console.log(wee2));
          
          break;
        case 'RETRIEVE_CONTACTS_RESPONSE':
          logger.warn(node.id, 'received contacts response');
          logger.debug(`${node.id} received RETRIEVE_CONTACTS_RESPONSE`);
      ...
      if (foundAt !== -1) {
        ...
        p.then(() => {
          return node.ping(bucket[0]).then(null, result => {
            bucket.splice(0);
            bucket.push(contact);
          }, wee3 => console.log(wee3));
        }, wee4 => console.log(wee4));
      }
      
      p.then(() => { bucket.sort(e => e.lastSeen); }, wee5 => console.log(wee5))
      
      return p.then(() => { return contact }, wee6 => console.log(wee6));
    };
    ...
    const getNearestNodesP = (contactID) => {
      ...
      }, wee7 => console.log(wee7));
    };
    
    const iteritiveFindP = (state, contacts) => {
      console.log(constants.test,': CONTACTS:');
      constants.test += 1;
    ...
    const refreshBucketsBeyondClosestP = (shortlist) => {
      ...
      refreshBuckets.forEach(index => {
        ...
        }, wee9 => console.log(wee9));
      });

      p.then(r => {
        console.log('refreshed all buckets')
        return r
      })
    ...
    const refreshBucketP = (index) => {
      ...
      console.log('calling getNearestNodesP from refreshBucket')
      return getNearestNodesP(random.toString('hex')).then(r => {
        console.log('finished getNearestNodesP in refreshBucket')
        return r
      });
    };
