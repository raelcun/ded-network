      ...
      switch(command.command)
      {
        ...
        case 'KEY_REQUEST':
          ...
          const requestedKey = routePublicKeyP(command.payload.state).then(res =>
            sendCommand(Command.createPubKeyRes(command, node.asContact(), res)))
          return
        case 'KEY_RESPONSE':
        ...
        case 'RETRIEVE_CONTACTS':
          ...
          updateContactP(command.payload.state.contact).then(result => {
            ...
            let { contacted, contactlist, closestNode, previousClosestNode } = state
            ...
            // check to see if closes node has changed
            let closestNodeUnchanged = closestNode === previousClosestNode
            if(closestNode && previousClosestNode) {
              closestNodeUnchanged = closestNode.id === previousClosestNode.id
            }
            
            // nearest contacts to contact list
            const nearestContacts = getNearestContacts(state.key, state.limit, node.id)
            contactlist = _.chain(contactlist).concat(nearestContacts).uniqBy('id').filter(contact => contact.id !== state.contact.id)
            
            // stop if conact list is full or the closest node didn't change
            const contactlistFull = contactlist.length >= constants.K
            ...
            state.remainingContacts = contactlist.filter(e => !_.includes(contacted, e.id))
