...
const serverOpts = { username: serverName, ip: '127.0.0.1', port: 3100, logger };
...
const main = async (num) => {
  ...
  if (num == '0'){
    ...
    node = await Node({ username: num, ip: '127.0.0.1', port: 3100 + Number(num), logger });
  ...
  process.stdin.on('data', async text => {
    ...
    if (input[0] == 'key'){
      const key = await node.findPublicKeyP(input[1]);
      console.log(key);
    }
    ...
    if(input[0] == 'message'){
      ...
      const key = await node.findPublicKeyP(input[1]);
      const response = await node.sendMessage(input[1], message, key);
