...
const Node = options => {
  ...
  return Router(newNode).then(router => {
    ...
    const sendMessage = (node, strMessage) => newNode.router.sendCommand(Command.createMessageReq(newNode, node, strMessage));
    const find = node => newNode.router.sendCommand(Command.createFindReq(newNode, node, newNode.publicKey));
