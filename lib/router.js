...
module.exports = node => {
  return new Promise((resolve, reject) => {
    ...
    const handleCommand = (serializedData) => {
      ...
      switch(command.command)
      {
        ...
        case 'RETRIEVE_CONTACTS':
          ...
          state.contacted[node.id] = updateContact(node.asContact());
          
          var contacts = getNearestContacts(node.asContact());
          
          console.log(buckets);
          
          state.contactList = state.contactList.concat(contacts);
          state.contactList = _.uniq(state.contactList, false, 'id');
          ...
          state = iteritiveFind(state, remainingContacts);
          command.payload.state = state;
          sendCommand(Command.createRetrieveContactRes(command, node.asContact()));
    ...
    const getNearestNodes = (contact) => {
      ...
      getNearestContacts(contact.id, state.limit, node.id).forEach(function addContact(contact) {state.contactlist.push(contact);});
