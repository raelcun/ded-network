...
const createMessageReq = (sourceNode, destNode, messageString) => {
  return createCommand(destNode, 'MESSAGE', {
    sourceId: sourceNode.id,
    sourceIP: sourceNode.ip,
    sourcePort: sourceNode.port,
    messageString // TODO: encrypt & sign string (or command)
  });
};
...
const createFindRes = (messageId, sourceNode, destinationId, destinationIP, destinationPort, publicKey) => {
  ...
  return createCommand(Node(nodeOpts), 'FIND_RESPONSE', {sourceId: sourceNode.id, destinationId: destinationId, publicKey}, messageId);
};
