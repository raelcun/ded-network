...
module.exports = node => {
  return new Promise((resolve, reject) => {
    ...
    const routePublicKeyP = async state => {
      ...
      const keys = _.flatten(routeResults).filter(e => e !== undefined && e !== null);
      const keyCounts = _.countBy(keys);
      // console.log('keys:');
      // console.log(keys);
      ...
      if (keys.length > 0){
        ...
        if ((keyCounts[maxKey] / keys.length * 100) < constants.APPROVAL_PERCENTAGE){
          // console.log('denied');
        }
        return [maxKey];
      }
