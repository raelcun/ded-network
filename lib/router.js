...
module.exports = async({ sourceContact, privateKey, viewCommands, logger: logger = Logger() }) => {
	...
	const sendMessage = async (username, message) => {
		...
		const queryId = utils.generateMessageId()
		const encryptedMessage = crypto.encrypt(publicKey, JSON.stringify({message: message, message_id: queryId, from_username: sourceContact.username, to_username: username, date_received: Date.now()}))
