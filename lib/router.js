...
module.exports = async ({ sourceContact, logger: logger = Logger() }) => {
	...
	const rpc = await RPC({ contact: sourceContact, logger, handleCommand: async command => {
		if (command.strCommand === 'RETRIEVE_CONTACTS') {
			...
			updateContact(command.payload.state.contact)
      ...
      const lookupState = await iterativeFind(state.key, state)
			...
			rpc.sendCommand(Command.createRetrieveContactsRes({ sourceContact, destContact: Contact({ id: destId, ip: destIp, port: destPort, username: destUsername }), orgRequestId: command.id, state: lookupState }))
		}
	}})
  ...
  const refreshBucketsBeyondClosest = async () => {
    logger.trace('entered refreshBucketsBeyondClosest')
    const bucketIndexes = Object.keys(buckets)
    const leastBucket = _.min(bucketIndexes)
    
    const refreshBuckets = bucketIndexes.filter(e => e >= leastBucket)
    for (let index of refreshBuckets) {
      const random = magic.getRandomInBucketRangeBuffer(index)
      await lookup({ contactID: random.toString('hex') })
    }
    
    logger.trace('completed refreshBucketsBeyondClosest')
  }
