...
const RPC = ({ contact, privateKey, handleCommand, logger = Logger() }) => {
	...
	const serverSocket = net.createServer(socket => {
		...
		socket.on('data', chunk => {
			...
			while (dIndex > -1) {
				const encryptedCommand = Command.deserialize(data.substring(0, dIndex))
				const command = Command.decrypt(encryptedCommand, privateKey)
				logger.debug(`${contact.username} received ${command.strCommand} command from ${command.payload.sourceUsername}`)
				if (_.includes(['MESSAGE_RESPONSE', 'RETRIEVE_CONTACTS_RESPONSE', 'KEY_REQUEST_RESPONSE'], command.strCommand)) {
	...
	const sendCommand = (command, publicKey) => {
		return new Promise((resolve, reject) => {
			const encryptedCommand = Command.encrypt(command, privateKey, publicKey)
			...
			socket.on('connect', () => {
				socket.write(Command.serialize(encryptedCommand) + messageDelimiter, () => {
					logger.info(`${contact.username} successfully sent\n${prettyjson.render(command, { noColor: true })}`)
					socket.destroy()
				})

				if (_.includes(['MESSAGE', 'RETRIEVE_CONTACTS', 'KEY_REQUEST'], command.strCommand)) {
