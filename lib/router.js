  ...
  return new Promise((resolve, reject) => {
    ...
    const handleCommand = (serializedData) => {
      ...
      switch(command.command)
      {
        ...
        case 'RETRIEVE_CONTACTS':
          ...
          state.contacted[node.id] = updateContact(state.contact);
          ...
          logger.warn('closestNode: ' + state.closestNode);
          logger.warn('previousClosestNode: ' + state.previousClosestNode);
          ...
          iteritiveFind(state, remainingContacts.splice(0, constants.ALPHA)).then(resultState => {
            state = resultState;
            command.payload.state = state;
            sendCommand(Command.createRetrieveContactRes(command, node.asContact()));
    ...
    const updateContact = (contact) => {
      ...
      if (_.includes(bucket.map(e => e.id), contact.id)) {
        ...
        // TODO: Eli should add this back in and make sure it works
    ...
    const getNearestNodes = (contactID) => {
      var state = {
        key: node.id,
      ...
      return iteritiveFind(state, state.contactlist).then(resultState => {
        state = resultState;
    ...
    const refreshBucket = (index) => {
      ...
      console.log(random);
      return getNearestNodes(random.toString('hex'));
    };
