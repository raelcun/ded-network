  ...
  return new Promise((resolve, reject) => {
    ...
    const handleCommand = async (serializedData) => {
      ...
      switch(command.command)
      {
        ...
        case 'KEY_REQUEST':
          ...
          queryCache.push(command.payload.state.queryId);
    ...
    const findPublicKeyP = async id => {
      ...
      if (bucket) {
        ...
        if (contactIndex !== -1) {
          return bucket[contactIndex].publicKey
        }
      }
      // send request to other nodes
      const queryId = utils.generateMessageId();
      queryCache.push(queryId);
      return await routePublicKeyP({ destinationId: id, visited: [], queryId });
    }

    const routePublicKeyP = async state => {
      console.log('routePublicKeyP')
      state.visited.push(node.id)
      const routes = await route(state, () => {
        console.log('found destination')
        return node.publicKey // return public key when it gets to destination node
      }, () => {
        ...
        return _.sampleSize(contactList, constants.ELI_THRESHOLD)
      }, contact => {
    ...
    const refreshBucketsBeyondClosestP = async (shortlist) => {
      ...
      for (let i = 0; i < bucketsToRefresh.length; i++) {
        await refreshBucketP(i)
      }

      bucketsToRefresh.forEach(index => {
        p = p.then(() => {
