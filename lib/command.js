...
const createPingRes = (request, sourceContact) => {
  ...
  return createCommand(Contact({ ip: request.payload.sourceIP, port: request.payload.sourcePort }), sourceContact, 'PING_RESPONSE', {
    sourceId: sourceContact.id,
...
const createMessageRes = (request, sourceContact) => {
  ...
  return createCommand(Contact({ ip: request.payload.sourceIP, port: request.payload.sourcePort }), sourceContact, 'MESSAGE_RESPONSE', {
    sourceId: sourceContact.id,
    destinationId: request.payload.sourceId
  }, request.id);
};
...
const createFindRes = (request, sourceContact) => {
  ...
  return createCommand(Contact({ ip: request.payload.sourceIP, port: request.payload.sourcePort }), sourceContact, 'FIND_RESPONSE', {
    sourceId: sourceContact.id,
    destinationId: request.payload.sourceId,
...
const createRetrieveContactRes = (request, contact) => {
  ...
  return createCommand(Contact({ ip: request.payload.sourceIP, port: request.payload.sourcePort }), contact, 'RETRIEVE_CONTACTS_RESPONSE', {
    sourceId: contact.id,
    destinationId: request.payload.sourceId,
