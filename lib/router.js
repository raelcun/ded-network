...
module.exports = async({
	...
	const rpc = await RPC({
		...
		handleCommand: async command => {
			if (command.strCommand === 'RETRIEVE_CONTACTS') {
				...
				console.log('Retrieve Contacts getNearestContacts')
				console.log(getNearestContacts(state.key, state.limit, sourceContact.id).map(e => e.username))
	...
	const iterativeFind = async(contactID, state) => {
		while (true) {
			...
			console.log(state.contactlist.map(e => e.username))
			console.log('Contacted')
			console.log(state.contacted)
